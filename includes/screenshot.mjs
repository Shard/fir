...
export async function process(screenshotCanvas, iconModelURL, iconClassNames, quantityModelURL, quantityClassNames) {
  ...
  return normalizeStockpile(stockpile, screenshotCanvas);
}

function normalizeStockpile(stockpile, screenshotCanvas) {
  const box = stockpile.bounds;
  const boxCanvas = cropCanvas(screenshotCanvas, box);

  const contents = stockpile.contents
    .filter(element => element.icon.CodeName)
    .map(element => {
      const iconBounds = element.icon.bounds;
      const iconBox = {
        x: iconBounds.x - box.x,
        y: iconBounds.y - box.y,
      ...
      return {
        ...
        quantity: element.quantity.value || 0,
        iconBox: {
          ...iconBox,
          canvas: cropCanvas(boxCanvas, iconBox),
        },
      };
    });
  ...
  return {
    ...
    header: {
      type: stockpile.header?.stockpile_type?.value || undefined,
      name: stockpile.header?.stockpile_name?.value || undefined,
    },
    contents,
  };
}
